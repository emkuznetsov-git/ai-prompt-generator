...
let config = {
  ...
  "format": {
    ...
    "platform": {
      ...
      "options": ["Instagram feed", "Instagram story/reels", "Print cover", "Magazine editorial", "Web banner", "Universal"]
    },
    "aspectRatio": {
      "label": "Aspect ratio (для полного роста лучше вертикальные)",
      "options": [
        {"value": "2:3 (вертикальный, полный рост)", "aspect": "2:3", "fullLength": true},
        {"value": "4:5 (Instagram feed)", "aspect": "4:5", "fullLength": true},
        {"value": "9:16 (Story/Reels)", "aspect": "9:16", "fullLength": true},
        {"value": "1:1 (квадрат)", "aspect": "1:1", "fullLength": false},
        {"value": "16:9 (горизонтальный)", "aspect": "16:9", "fullLength": false}
      ]
    },
    "options": [
  ...
  "camera": {
    ...
    "framing": {
      "label": "Framing (управление полным ростом)",
      "shotType": {
        "label": "Shot type",
        "options": ["full-length", "long shot", "full body", "full body in frame", "head-to-toe"]
      },
      "cropGuards": {
        "label": "Crop guards",
        "options": ["head-to-toe", "feet visible", "no cropped head", "no cropped feet", "shoes visible", "include floor", "leave space above head and below feet"]
      },
      "subjectScale": {
        "label": "Subject scale in frame",
        "options": ["subject fits entirely with margin", "small in frame", "medium in frame", "full body with breathing room"]
      },
      "lensDistance": {
        "label": "Lens/distance",
        "options": ["35mm, camera pulled back", "50mm, wide framing", "camera pulled back", "wide framing", "long shot distance"]
      },
      "anchorDetails": {
        "label": "Anchor details",
        "options": ["show shoes", "show floor", "feet on ground visible", "full body from head to toes"]
      }
    }
  },
  "lighting": {
...
function initializeUI() {
    ...
    // Новые поля формата
    if (config.format.platform) {
        populateSelect('platform', config.format.platform.options);
    }
    if (config.format.aspectRatio) {
        populateSelect('aspectRatio', config.format.aspectRatio.options, 'value');
    }
    ...
    // Framing (новые поля для полного роста)
    if (config.camera.framing) {
        populateSelect('framingShotType', config.camera.framing.shotType.options);
        populateSelect('framingCropGuards', config.camera.framing.cropGuards.options);
        populateSelect('framingSubjectScale', config.camera.framing.subjectScale.options);
        populateSelect('framingLensDistance', config.camera.framing.lensDistance.options);
        populateSelect('framingAnchorDetails', config.camera.framing.anchorDetails.options);
    }
    ...
    toggleOutputBtn.addEventListener('click', () => {
        if (outputPanel.classList.contains('collapsed')) {
            ...
            outputPanel.classList.add('expanded');
            toggleOutputBtn.textContent = '▼';
            toggleOutputBtn.classList.remove('rotated');
            document.body.classList.add('output-expanded');
        } else {
            outputPanel.classList.remove('expanded');
            outputPanel.classList.add('collapsed');
            toggleOutputBtn.textContent = '▲';
            toggleOutputBtn.classList.add('rotated');
...
function generatePrompt() {
    const mainParts = [];
    const negativeParts = [];
    const framingPreface = [];
    
    // ========== FRAMING-PREFACE (в самое начало промпта) ==========
    const framingShotType = document.getElementById('framingShotType')?.value;
    const framingCropGuards = document.getElementById('framingCropGuards')?.value;
    const framingSubjectScale = document.getElementById('framingSubjectScale')?.value;
    const framingLensDistance = document.getElementById('framingLensDistance')?.value;
    const framingAnchorDetails = document.getElementById('framingAnchorDetails')?.value;
    
    if (framingShotType || framingCropGuards || framingSubjectScale || framingLensDistance || framingAnchorDetails) {
        const framingParts = [];
        
        if (framingShotType) {
            framingParts.push(framingShotType.toUpperCase());
        }
        
        if (framingCropGuards) {
            framingParts.push(framingCropGuards);
        }
        
        if (framingSubjectScale) {
            framingParts.push(framingSubjectScale);
        }
        
        if (framingLensDistance) {
            framingParts.push(framingLensDistance);
        }
        
        if (framingAnchorDetails) {
            framingParts.push(framingAnchorDetails);
        }
        
        // Добавляем стандартные требования для полного роста
        framingParts.push('full body in frame', 'feet visible', 'shoes visible', 'include floor', 'leave space above head and below feet');
        
        framingPreface.push(framingParts.join(', '));
    }
    ...
    // Framing negatives (дублируем в negatives для надежности)
    if (framingShotType || framingCropGuards) {
        const framingNegatives = [];
        framingNegatives.push('no close-up', 'no portrait crop', 'no half-body');
        
        if (framingCropGuards) {
            if (framingCropGuards.includes('feet') || framingCropGuards.includes('head')) {
                framingNegatives.push('no cropped feet', 'no cropped head');
            }
        } else {
            framingNegatives.push('no cropped feet', 'no cropped head');
        }
        
        negativeParts.push(...framingNegatives);
    }
    ...
    if (aspectRatio) {
        const aspectObj = config.format.aspectRatio.options.find(f => f.value === aspectRatio);
        if (aspectObj && aspectObj.aspect) {
            negativeParts.push(`--ar ${aspectObj.aspect}`);
        }
    } else if (format) {
    ...
    // Framing-preface в самое начало промпта
    if (framingPreface.length > 0) {
        prompt = framingPreface.join('. ') + '.\n\n';
    }
    
    // Основной промпт
    prompt += mainParts.join(', ');
...
function generateRandomPrompt() {
    ...
    // Основные настройки
    if (config.format.platform) {
        randomSelect('platform', config.format.platform.options);
    }
    if (config.format.aspectRatio) {
        randomSelect('aspectRatio', config.format.aspectRatio.options);
    }
    ...
    // Framing (для полного роста)
    if (config.camera.framing) {
        randomSelect('framingShotType', config.camera.framing.shotType.options);
        randomSelect('framingCropGuards', config.camera.framing.cropGuards.options);
        randomSelect('framingSubjectScale', config.camera.framing.subjectScale.options);
        randomSelect('framingLensDistance', config.camera.framing.lensDistance.options);
        randomSelect('framingAnchorDetails', config.camera.framing.anchorDetails.options);
    }
